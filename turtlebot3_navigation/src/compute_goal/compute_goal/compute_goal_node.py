...
class ComputeGoalNode(Node):
    def __init__(self):
        ...
        self.tf_buffer = Buffer()       # For TFs
        self.tf_listener = TransformListener(self.tf_buffer, self)      # For buffer updates
        
        # Publishes PoseStamped over /goal
        self.publisher = self.create_publisher(PoseStamped, '/goal', 10)
        self.target_frame = 'map'
        # Use the two given AprilTags to get the midpoint
        self.tag_frames = ['tag36h11:1', 'tag36h11:10']

        self.goal_published = False     # Avoid publishing multiple goals 
    ...
    def timer_callback(self):
        ...
        if not all_tags_available:
            # Wait if one or more tags still missing
            self.get_logger().info("Waiting for both tag TFs to be available...")
            return  

        for tag_frame in self.tag_frames:
            # Get 3D translation of tag in map frame
            transform = self.tf_buffer.lookup_transform(
                self.target_frame,
                tag_frame,
                now,
                timeout=Duration(seconds=1.0)       # Wait 1s for the TF
            )
        ...
        goal.pose.orientation = Quaternion(x=0.0, y=0.0, z=0.0, w=1.0)      # No rotation
